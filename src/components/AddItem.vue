...
<!--  כלומר הוי מודל עוזר לנו לעשות תהליך שנקרא ביינדינג בעצם לקשר את מה שיכנס לאינפוט לדאטה-->
  ...
  <q-select outlined v-model="newRecipe.type" :options="typeOptions" style="max-width: 200px" class="q-mb-lg" label="Type" >
  ...
  <q-select outlined v-model="newRecipe.difficulty" :options="diff" style="max-width: 200px" class="q-mb-lg" label="Difficulty" >
    <template v-slot:prepend>
...
export default {
  ...
  methods: {
    ...
    update() {
      localStorageDriver.update(this.tableName, this.newRecipe.id, this.newRecipe);
      this.$router.push(`/`);
    },
    addIngredient(){
      debugger
